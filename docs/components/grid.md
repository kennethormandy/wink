...
Unlike the [Foundation grid](http://foundation.zurb.com/docs/components/grid.html), Ink's grid cannot be nested. A layout simulating rows within columns is possible, however, by placing multiple `.columns` tables (with the same number of columns) within the same `.wrapper` td.
 -->
